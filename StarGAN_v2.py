...
# from tensorflow.data.experimental import AUTOTUNE, prefetch_to_device
...
class StarGAN_v2():
    def __init__(self, args):
        ...
        # exponential moving average: smoothing weight EMAt = ema_decay * EMA t-1 + (1-ema_decay)* value t
        self.ema_decay = args.ema_decay

        """ Weight """
        self.adv_weight = args.adv_weight  # adversarial loss
        self.sty_weight = args.sty_weight  # style consistency loss
        self.ds_weight = args.ds_weight  # diversity sensitive loss
        self.cyc_weight = args.cyc_weight  # cycle consistency loss

        # gradient penalty / in discriminator use gp method and update weight
        ...
        self.num_style = args.num_style  # types the network can generate

        """ Mapping Network """
        self.hidden_dim = args.hidden_dim

        """ Discriminator """
        self.sn = args.sn  # spectral normalization
    ...
    def build_model(self):
        ...
        def train(self):
            ...
            for idx in range(self.start_iteration, self.iteration):
                ...
                g_adv_loss_ref, g_sty_loss_ref, g_ds_loss_ref, g_cyc_loss_ref, g_loss_ref = self.g_train_step(x_real,
                                                                                                              y_org,
                                                                                                              y_trg,
                                                                                                              x_refs=[
                                                                                                                  x_ref,
        ...
        def refer_canvas(self, x_real, x_ref, y_trg, path, img_num):
            ...
            canvas = PIL.Image.new('RGB',
                                   (self.img_size * (src_img_num + 1) + 10, self.img_size * (ref_img_num + 1) + 10),
                                   'white')
